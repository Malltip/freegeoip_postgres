...
if __name__ == "__main__":
    ...
    curs.execute("drop table if exists country_blocks")
    curs.execute("""\
    ...
    curs.execute("drop table if exists region_names")
    curs.execute("""\
    create table region_names (
        country_code text,
        region_code text,
        region_name text,
        region_name_id serial8,
        unique (country_code, region_code))""")
    import_csv(curs, region_csv, "region_names")

    curs.execute("drop table if exists city_blocks")
    curs.execute("""\
    ...
    curs.execute("drop table if exists city_location")
    curs.execute("""\
    ...
    count = 0
    regions = conn.cursor()
    regions.execute("SELECT * FROM region_names")

    for region in regions:
        region_name = region[-2]  # country_code,region_code,region_name, region_name_id
        country_name = world_countries.get(region[0])
        ...
        if country_name:
            ...
            if k in world_regions:
                ...
                if region_name != new_name:
                    update = conn.cursor()
                    update.execute("UPDATE region_names SET region_name=%s "
                                   "WHERE region_name_id=%s", (new_name, region[-1]))
                    update.close()
                    count += 1
